...
const BooksPageComponent = () => {
    ...
    return <Box mb={20}>
        <Text textAlign={'center'} pt={5}>
            <SectionTitle title={t('title', {ns: 'books'})}
        ...
        <Flex justify={'center'} mt={5} flexWrap={'wrap'}>
            {booksCategory.map((item, idx) => (
                <Button colorScheme={'facebook'}
                        ...
                        key={item.id}>{t(item.label, {ns: 'books'})}
                </Button>
            ))}
        </Flex>
        <Grid gridTemplateColumns={{base: 'repeat(1, 1fr)', md: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)'}} rowGap={20} gap={5} mt={5} >
            {filteredData().map(item => (
                <motion.div key={item.name} layout animate={true} >
                    <Box key={item.name} pos={'relative'}>
                        <Image src={item.image} alt={item.name} w={'full'} h={'250px'} borderRadius={'lg'}
                               objectFit={"cover"} />
                        <HStack justify={'space-between'} pos={'absolute'} minH={'90px'} borderRadius={'lg'} boxShadow={'dark-lg'}
                                bg={backgroundColor} left={2} right={2} bottom={-10} p={2}>
                            <Box>
                                <Text fontSize={'md'}>{item.name}</Text>
                                <Text fontWeight={'bold'} fontSize={'2xl'}>{item.price.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}</Text>
                            </Box>
                            <Button rightIcon={<AiFillShopping />} colorScheme={'facebook'}>Buy</Button>
                        </HStack>
                    </Box>
                </motion.div>
                ))}
